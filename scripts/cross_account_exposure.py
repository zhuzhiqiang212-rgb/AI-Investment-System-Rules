...
def build(date_h):
    ...
    for code, m in merged.items():
        ...
        rows.append({
            ...
            "合并跨度贡献pp": round(m["跨度贡献_raw"] * 100, 2), "置信度": m["置信度"],
            "锚龄天数": _age(today, pa) if pa else None, "blind": m["blind"],
        })
    ...
    out = {"_说明": "跨账户单只合并暴露(S1·轮55)。★只给暴露事实·不给操作建议(S1-3)。合并权重=跨账户合并市值÷主战场合计A(富途+SBI)。",
           "date": date_h, "主战场合计A_USD": round(A_total, 2),
           "富途A_USD": round(A_futu, 2), "SBI_A_USD": round(A_sbi, 2), "单只上限": SINGLE_MAX,
           "跨账户合并(按合并权重降序)": rows,
           "最大单一暴露": {"标的": top.get("name"), "code": top.get("code"), "合并权重pct": top.get("合并权重pct"),
                        "破单只20%": top.get("破单只20%")},
           "破单只20%的标的": [r["name"] for r in rows if r["破单只20%"]],
           "IBKR_bitFlyer附录": appendix,
           "★口径": "高收益(合并跨度贡献大)·高波动(同上)·低把握(置信度C/锚龄大/blind)三件集中在同一只→一眼可见·是否动仓是Opus5/董事长裁定。"}
