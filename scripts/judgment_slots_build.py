...
def _empty_slot(ev_ids):
    return {"本层判断": None, "依据": None, "证伪信号": None,
            ...
            "★依据右栏哪把尺(①~⑥·E8必填)": None,
            "_依据右栏哪把尺_允许值": ["①世界观", "②国家战略", "③资金流动", "④板块地图", "⑤过滤五关", "⑥持仓档案"],
            "_依据右栏哪把尺_格式要求": "★必须以 ①~⑥ 层号开头(可在层号后加括注/组合·但首字符须是层号)。轮338:right_left_gate 严格判前缀·非关键词包含·填不对退回改填(不放松闸)。",
            "★与尺是否矛盾(E8枚举·必填)": None,
            "_与尺是否矛盾_允许值": ["无矛盾", "★修正判断", "★提改尺"],
            "_与尺是否矛盾_格式要求": "★必须【完全等于】上三个值之一(不认『不矛盾』等同义词·轮338 right_left_gate 严格枚举)。无冲突填『无矛盾』；判断向尺靠填『★修正判断』；要改尺填『★提改尺』(须董事长签字)。",
