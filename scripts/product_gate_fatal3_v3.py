#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""gate_fatal3_v3·三致命区A/B/C/D + 4扩查(补硬闸盲区·GPT复验locked_v3)。对locked_v3跑。
...
P = ROOT / "00_请先看这里" / "★每日产品_2026-07-22_locked_v3.html"
raw = P.read_bytes()
h = raw.decode("utf-8")
...
for tok, desc in [("目标价¥33,544", "爱德万目标价"), ("¥33,544(+6.5%)", "爱德万目标价+上行%"), ("超上沿约1.9倍", "爱德万倍数"),
    ...
    for m in re.finditer(re.escape(tok), h):
        if not hist(m.start()):
            ...
            break
# 扩③补盲区:前瞻预测表/一句话/决定摘要/任何区 爱德万/闪迪邻近 估值=合理/极贵/目标价/加仓价/倍数(当日非hist·非异常待核)
for m in re.finditer(r"估值[=＝](?:合理|极贵)|估值合理|目标价[¥$][\d,]+|加仓价[¥$][\d,]+|[\d.]+倍(?!数据|以)", h):
    if hist(m.start()):
        continue
    ctx = pl(h[max(0, m.start() - 46):m.start() + 12])
    if ("爱德万" in ctx or "闪迪" in ctx) and "异常" not in ctx and "待核" not in ctx and "不计算" not in ctx:
        F["扩③爱德万闪迪异常字段残留"].append({"区": "前瞻/一句话/决定摘要", "命中": pl(h[m.start():m.start() + 12]), "上下文": ctx[-40:]})
...
print("=== gate_fatal3_v3(三致命区A/B/C/D + 4扩查·对locked_v3) ===")
for k, v in F.items():
    print(f"  [{'PASS' if not v else 'FAIL'}] {k}: {v if v else '无'}")
print(f"★全PASS = {all_pass}")
print(f"--- 版本 --- 字节:{len(raw)} · mtime:{mtime} · SHA256:{sha}")
(ROOT / "data/screen/gate_fatal3_v3.json").write_text(json.dumps({
    "★本报告核对版本SHA": sha, "版本号": "v3", "file": P.name, "字节": len(raw), "mtime": mtime,
    "ABCD+4扩查_FAIL": F, "全PASS": all_pass}, ensure_ascii=False, indent=1) + "\n", encoding="utf-8")
print("落: data/screen/gate_fatal3_v3.json")
