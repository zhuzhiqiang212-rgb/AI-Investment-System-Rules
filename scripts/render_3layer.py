...
_ANOM_DUALTRACK = {
    "6857": ('<span class="dualtrack" style="color:#c9a86a">★这只股用两把尺算出相反结论（分歧本身就是它的投资分歧）：'
             ...
             '★但爱德万自身5年PE区间11~100+（2022低11~17→2024-25高100+）·无稳定"正常化PE"锚→前瞻这把尺的PE也只能取当前峰值周期水平；'
             '若周期回落到历史低PE（11~17），公允将降到约¥5,000~7,000。'
             '分歧点＝AI测试设备需求是【结构性台阶】还是【周期峰值】——两把尺、连前瞻尺的PE，最终都压在这一个问题上。答案出来前不给单一贵贱结论。</span>'),
}
