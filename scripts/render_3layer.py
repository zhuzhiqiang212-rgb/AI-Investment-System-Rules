...
_ANOM_DUALTRACK = {
    "6857": ('<span class="dualtrack" style="color:#c9a86a">★这只股用两把尺算出相反结论（分歧本身就是它的投资分歧）：'
             '① 中周期尺（回望均值·剔除FY26峰值EPS¥513.30）：公允约¥2,940 → 现价¥28,945＝极贵（约10倍）；'
             '② 前瞻尺（EDINET真实EPS¥413.29 × 合理PE 35~50·AI测试双寡头结构成长）：公允约¥14,500~¥20,700 → 现价约1.4~2倍＝偏贵但非极端。'
             '分歧点＝AI测试设备需求是【结构性台阶】还是【周期峰值】。'
             '★在这个问题有答案之前，不给单一贵贱结论——两把尺都摆出来，由基本面（下季财报/HBM需求/订单）去回答。</span>'),
}
