...
AI_SUPPLY_NODE_CLASSES = {"算力", "芯片", "电力", "半导体设备", "存储", "代工", "盟友节点", "盟友", "盟友链", "AI软件应用", "软件云", "AI应用"}  # 轮10 A1:加"芯片"(博通改归芯片后仍属AI供应链)·顺加"软件云/AI应用"(尺7环节用词·为A2单环节做准备·不改AI供应链口径)
...
HOLDING_NODE_MAP = {
    ...
    # 轮10 A1(架构师裁定):博通AVGO=定制AI芯片(ASIC)+网络芯片设计·非测试设备→由"半导体设备"改归"芯片"(爱德万6857=测试机·仍为设备)
    "AVGO": "芯片", "6857": "半导体设备", "TSM": "代工", "SNDK": "存储",
    "9984": "盟友链",
    # 加密
...
#   AI供应链设999=不再作硬上限拦(仅展示占比);单一标的上限改20%(规矩1)。四规矩在产品「风险配仓」模块显性显示。
CONC_UPPER_LIMITS = {"AI供应链": 999.0, "单一标的": 20.0, "加密": 12.0}
